...
const LotteryCards: React.FC<Props> = (props) => {
  ...
  const activeLotteries = getAllLottery?.filter((Lottery: { Tickets: []}) => Lottery.Tickets.includes(address));
  const ownerLotteries = getAllLotteryFromAddress?.filter((Lottery: { Owner: any}) => Lottery.Owner==address);
console.log("active", activeLotteries)
