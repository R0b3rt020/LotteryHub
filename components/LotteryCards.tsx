...
const LotteryCards: React.FC<Props> = (props) => {
  ...
  const handleBuyTickets = async (lotteryKey:Number,CostPerTicket:Number) => {
    ...
    if(isMismatched){
      ...
      toast.error("Please switch your wallet to Pulse Chain testnet")
      return
    }
  ...
  <div className="w-full sm:w-26/12">
    <div className="p-2">
      <div className="p-4 rounded-3xl bg-green-200">
        ...
        <div>
          <p className="pt-4 text-sm font-bold m-0sm:text-xs">Tickets left {(Number(lottery?.MaxTickets) - Number(lottery?.Tickets.length)).toString()} of {lottery?.MaxTickets.toString()}</p>
  ...
  <div className="w-full sm:w-26/12">
    <div className="p-2">
      <div className="p-4 rounded-3xl bg-gray-300">
        ...
        <h2 className='text-2xl text-center sm:text-xl'>Winner {lottery?.winner.toString().substring(0,5)}...{lottery?.winner.toString().substring(lottery?.winner.toString().length, lottery?.winner.toString().length - 5)}</h2>
